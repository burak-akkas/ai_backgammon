...
public class bgAI extends ApplicationAdapter {
	...
	public void render () {
		...
		if(selector.getY() != 0 && selector.getX() != 0) {
			...
			if(fromIndex == 24 || fromIndex == 25) { selector.setHeight(40); selector.setWidth(40); }
	...
	public void update() {
		if(board.checkWinner() == 0) {
			if(fromSelected && toSelected) {
				...
				} else if((dices.contains(Math.abs(toIndex + 1)) && isWhite)
						|| (dices.contains(Math.abs(24 - (toIndex + 1))) && !isWhite)) {

					int current = 0;

					if(isWhite) { current = dices.indexOf(Math.abs(toIndex + 1)); }
					else { current = dices.indexOf(Math.abs(24 - (toIndex + 1))); }

					int currentDice = dices.get(current);

					if((board.getLogicBoard()[fromIndex] < 0 && !isWhite && board.reenter(fromIndex, toIndex, currentDice)
							|| (board.getLogicBoard()[fromIndex] > 0 && isWhite && board.reenter(fromIndex, toIndex, currentDice)))) {
